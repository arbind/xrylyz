...
class ThreadPool
  ...
  def initialize(size=8, name='pool')
    ...
    @pool = Array.new(@size) do |i|
      Thread.new do
        Thread.current[:id] = i
        Thread.current[:name] = "#{name}-thread#{i}"
        ...
        # Shutting down is merely a `#schedule { throw :exit }` away!
          ...
          loop do
            ...
            job, args = @jobs.pop
            puts "#{Thread.current[:name]} starting #{args}"
            job.call(*args)
            Thread.current[:num_jobs] = Thread.current[:num_jobs] + 1
            puts "#{Thread.current[:name]} finished #{args}"
          end
        end
      end
    end
...
# Demonstration
# -------------
# Running this method  will display how the thread pool works.
if $0 == __FILE__
